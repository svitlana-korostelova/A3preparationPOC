...
**Purpose**: AI-optimized execution guide for GitHub Copilot (Chat, coding agent, Copilot CLI) working on the **A3 Senior iOS Developer interview-prep PoC**.

Same project rules as `CLAUDE.md`. Copilot loads this file automatically. Copilot CLI also reads `CLAUDE.md` / `AGENTS.md` / `GEMINI.md` if present — keep this file in sync with `CLAUDE.md`.
...
| Item | Copilot CLI | Notes |
|------|-------------|-------|
| Invoke a skill | `skill` tool | Skills live under `.claude/skills/` (symlinks) and `.agents/skills/` |
| Read / write / edit files | `view` / `create` / `edit` | Claude Code names: `Read` / `Write` / `Edit` |
| Shell | `bash` | macOS zsh/bash syntax |
| Search | `grep` / `glob` | |
| Subagents | `task` (`agent_type`: `general-purpose` or `explore`) | Status: `read_agent`, `list_agents` |
| Todos | `sql` against built-in `todos` table | |
| Web | `web_fetch` | No `WebSearch` — fetch a search-engine URL |
| Tool map | `.agents/skills/using-superpowers/references/copilot-tools.md` | Use this when a skill names Claude tools |
...
| **Test** | *No test target yet* | Ask user before scaffolding one |
...
```
A3preparationPOC/                 # repo root (docs + AI agent config)
├── CLAUDE.md                     # Claude Code instructions (keep in sync)
├── .github/copilot-instructions.md  # this file
├── .codemie/guides/              # AI-optimized topic guides
├── .claude/skills/               # superpowers skills (symlinks)
├── .agents/skills/               # same skills (Copilot / agents path)
...
| Topic | File | Status |
|-------|------|--------|
| GCD | `Sources/GCDView.swift` | done |
| OperationQueue | `Sources/OperationQueueView.swift` | done |
| Semaphores | `Sources/SemaphoreView.swift` | done |
| Locks | `Sources/LocksView.swift` | done |
| Structured Concurrency | `Sources/StructuredConcurrencyView.swift` | done |
| pthreads | `Sources/PThreadsView.swift` | done |
| Combine | `Sources/CombineView.swift` | done |
...
When adding a planned topic, follow `architecture.md` section "Adding a New Interview Topic".
